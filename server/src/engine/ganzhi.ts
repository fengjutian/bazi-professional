...
export function getGanZhiIndex(ganZhi: string): number {
  ...
  if (index === -1) throw new Error(`Invalid GanZhi: ${ganZhi}`);
  return index;
}
...
// 计算月干支（基于年干和月份）
export function getMonthGanZhi(yearGan: TianGan, month: number): string {
  // 年干对应的月干索引
  const monthGanMap: Record<TianGan, number> = {
    '甲': 1, '乙': 3, '丙': 5, '丁': 7, '戊': 9,
    '己': 1, '庚': 3, '辛': 5, '壬': 7, '癸': 9
  };
...
// 计算时干支（基于日干和时辰）
export function getTimeGanZhi(dayGan: TianGan, hour: number): string {
  // 日干对应的时干索引
  const timeGanMap: Record<TianGan, number> = {
    '甲': 1, '乙': 3, '丙': 5, '丁': 7, '戊': 9,
    '己': 1, '庚': 3, '辛': 5, '壬': 7, '癸': 9
  };
  ...
  return TIANGAN[ganIndex] + DIZHI[timeIndex];
}